...
public class DiseaseInfoFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View fragmentView = inflater.inflate( R.layout.fragment_disease_info , container , false ) ;

        TextView textView = fragmentView.findViewById( R.id.t1 ) ;
        textView.setTextIsSelectable( true ) ;
        String s = getContext().getResources().getString(R.string.t1);
        textView.setText( Html.fromHtml( s ) );

        TitleViewModel viewModel = ViewModelProviders.of( getActivity() ).get( TitleViewModel.class ) ;
        viewModel.setTitle( getString( R.string.what_is_melanoma ) ) ;
